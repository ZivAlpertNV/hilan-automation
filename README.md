...
- **ממשק אינטראקטיבי** - הזנה מונחית ב-7 שלבים עם לוח שנה ויזואלי צבעוני
...
4. **Step 4/7 — Vacation** - סימון ימי חופשה, תומך בטווחים (`8-12`) וימים בודדים (`1,3,15`)
5. **Step 5/7 — Sick Days** - סימון ימי מחלה, כולל צירוף אישור מחלה אוטומטי ב-3+ ימים
6. **Step 6/7 — Office Days** - ימים קבועים בשבוע שבהם אתה במשרד (ברירת מחדל: שני ורביעי — `2,4`)
7. **Step 7/7 — Review** - לוח שנה צבעוני מלא → מעבר למסך עריכה
...
| 3 | Month | 10 | Vacation |
| 4 | Year | 11 | Sick days |
| 5 | Project | 12 | Headless |
| 6 | Entry (שעת כניסה) | 13 | Dry-run |
| 7 | Exit (שעת יציאה) | d | Done — סיום עריכה |
...
- **בדיקת חפיפות** - זיהוי אוטומטי של חפיפות בין חופשה/מחלה/נוכחות עם אזהרה מיידית
...
- **חפיפות** - בדיקה שאין חפיפה בין ימי חופשה, מחלה, ונוכחות (גם present-dates וגם present-days)
...
### Pass 1 - פעולות Postback (חופשה, מחלה, תיקון סמלים)
...
9. **תיקון סמלים** - מחיקת שורות absence שגויות ותיקון presence/w.home
...
10. **מילוי שעות** - ממלא שעת כניסה ויציאה לכל יום עבודה (א'-ה') דרך JavaScript
11. **Reporting Type** - מוודא "presence" בימי נוכחות, "w.home" בשאר
12. **מילוי פרויקט** - ממלא את קוד הפרויקט דרך autocomplete (עם retry אוטומטי עד 3 ניסיונות + JS fallback)
13. **Retry פרויקט** - ניסיון חוזר לשורות שנכשלו (בדרך כלל השורה הראשונה שבה ה-autocomplete עוד לא מאותחל)
14. **שמירה** - לוחץ Save לשמירת כל הנתונים
